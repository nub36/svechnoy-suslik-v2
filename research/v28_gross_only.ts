/**
 * V2.8 — PURE ALPHA / ZERO-FEE EXIT COMPARISON.
 *
...
async function main(): Promise<void> {
  ...
  for (const k of ARM_ORDER) acc.set(k, mk());
  const entryKeys = new Map<string, Set<string>>();
  for (const k of ARM_ORDER) entryKeys.set(k, new Set());
      ...
      if (open) {
        ...
        if (out) {
          if (open.sniper) {
            ...
            // --- fixed-RR arms.
            const rrBars = closed.slice(open.entryIndex,
              Math.min(closed.length, open.entryIndex + RR_MAX_BARS + 2));
            for (const label of ['RR15', 'RR20', 'RR25', 'RR30', 'RR40'] as const) {
              const r = simulateFixedRr(dir, open.entryPrice, open.stop,
                RR_MULTS[label]!, rrBars);
              if (!r) continue;
              record(acc.get(label)!, r.grossR, r.exit, r.exit === 'TP',
                r.barsHeld, b.symbol, b.timeframe, dir);
              entryKeys.get(label)!.add(key);
            }
          }
          open = null;
        }
  ...
  const perArm = Object.fromEntries(
    ARM_ORDER.map((k) => [k, entryKeys.get(k)!.size]));
  const identical = ARM_ORDER.every((k) => {
  ...
  const arms = ARM_ORDER.map((k) => summarise(k));
...
const invokedDirectly = process.argv.some((a) => a.includes('v28_gross_only'));

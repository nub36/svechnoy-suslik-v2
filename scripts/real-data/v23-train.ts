/**
 * V2.3 TRAIN driver — 6-arm ablation across 15m/30m/1h/4h.
 *
...
import { replayV23, V23_ARMS, type V23Trade } from './v23-replay';
...
interface Acc {
  ...
  byHtf: Map<string, Sub>; byPool: Map<string, Sub>; byTp2: Map<string, Sub>;
  // baseline linkage for anti-bias checks
  filledKeys: Set<string>; declinedKeys: Set<string>; guardRejKeys: Set<string>;
}
const mk = (): Acc => ({
  ...
  byHtf: new Map(), byPool: new Map(), byTp2: new Map(),
  filledKeys: new Set(), declinedKeys: new Set(), guardRejKeys: new Set(),
});
...
function add(a: Acc, t: V23Trade): void {
  ...
  bump(a.byTp2, t.tp2Source, g);
}

async function main(): Promise<void> {
  const cache = arg('cache');
  const outFile = arg('out');
  const armNames = arg('arms', Object.keys(V23_ARMS).join(',')).split(',');
  ...
  const out: Record<string, unknown> = {
    slice: 'train', scope: SCOPE, feeEnvironments: FEE_ENVS, arms: {},
  };
  ...
  for (const name of armNames) {
    ...
    const netFut4 = net('FUT_4'), netFut7 = net('FUT_7');
    M[name] = {
      gates: V23_ARMS[name],
      ...
      criterionFut4PerFilledPositive: netFut4.perFilled > 0,
      criterionFut4PerSetupPositive: netFut4.perSetup > 0,
      criterionFut7PerFilledPositive: netFut7.perFilled > 0,
      criterionFut7PerSetupPositive: netFut7.perSetup > 0,
      criterionAllPassed: netFut4.perFilled > 0 && netFut4.perSetup > 0
        && netFut7.perFilled > 0 && netFut7.perSetup > 0,
      outlierDependence: {
      ...
      antiBias: {
        ...
        baselineGrossOfFeeGuardRejected: bl(a.guardRejKeys),
      },
      byTimeframe: sub(a.byTf), byDirection: sub(a.byDir), bySymbol: sub(a.bySym),
      byHtfAlignment: sub(a.byHtf), byPoolKind: sub(a.byPool), byTp2Source: sub(a.byTp2),
    };
  }
